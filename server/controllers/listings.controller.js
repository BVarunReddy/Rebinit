...
async function createListing(req, res) {
  try {
    ...
    const [result] = await pool.query(
      "INSERT INTO listings (user_id, title, category, description, quantity, location, image_url) VALUES (?,?,?,?,?,?,?)",
      [req.user.id, title, category, description, quantity, location, imageUrl],
    );
...
async function deleteListing(req, res) {
  try {
    await pool.query("DELETE FROM listings WHERE id = ? AND user_id = ?", [
      req.params.id,
      req.user.id,
    ]);
