...
async function createListing(req, res) {
  try {
    const { title, category, description, quantity, location } = req.body;
    const imageUrl = req.file ? `/uploads/${req.file.filename}` : null;
    const [result] = await pool.query('INSERT INTO listings (user_id, title, category, description, quantity, location, image_url) VALUES (?,?,?,?,?,?,?)', [req.user.id, title, category, description, quantity, location, imageUrl]);
...
async function getAllListings(req, res) {
  try {
    ...
    let query = `SELECT l.*, u.name AS user_name FROM listings l JOIN users u ON u.id = l.user_id WHERE l.status = 'Available'`;
    const params = [];
